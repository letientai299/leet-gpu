...
bool ImageBytes::upload(const Image &input, std::size_t output_size) {
  ...
  if (!CUDA_CHECK(cudaMalloc(reinterpret_cast<void **>(&input_), input.size)) ||
      !CUDA_CHECK(
          cudaMalloc(reinterpret_cast<void **>(&output_), output_size_)) ||
      !CUDA_CHECK(cudaMemcpy(input_, input.pixels, input.size,
...
bool ImageBytes::download(Image &output, unsigned width, unsigned height) {
  ...
  return CUDA_CHECK(
      cudaMemcpy(output.pixels, output_, output_size_, cudaMemcpyDeviceToHost));
}

const unsigned char *ImageBytes::input() const { return input_; }

unsigned char *ImageBytes::output() { return output_; }
