...
__global__ static void vec_add_kernel(const float* a, const float* b, float* c, std::size_t count) {
...
static void
init_vectors(std::vector<float>& a, std::vector<float>& b, std::vector<float>& expected) {
...
static bool
add_on_gpu(const std::vector<float>& a, const std::vector<float>& b, std::vector<float>& c) {
  ...
  const unsigned blocks = static_cast<unsigned>((a.size() + threads - 1) / threads);
...
static bool verify_output(const std::vector<float>& c, const std::vector<float>& expected) {
